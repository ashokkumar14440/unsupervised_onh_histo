...
class SegmentationNet10aTrunk(VGGTrunk):
    def __init__(self, config, cfg):
        ...
        self.batchnorm_track = config.architecture.batchnorm_track

        assert config.dataset.parameters.input_size % 2 == 0
...
class SegmentationNet10aHead(nn.Module):
    def __init__(self, config, output_k, cfg):
        ...
        self.num_sub_heads = config.architecture.num_sub_heads
        ...
        self.input_size = config.dataset.parameters.input_size
    ...
    def forward(self, x):
        ...
        for i in range(self.num_sub_heads):
            x_i = self.heads[i](x)
            x_i = F.interpolate(
                x_i, size=self.input_size, mode="bilinear", align_corners=True
            )
            results.append(x_i)
