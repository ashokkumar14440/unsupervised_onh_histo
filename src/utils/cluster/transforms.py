...
def sobel_process_bak(imgs, include_rgb, using_IR=False):
    ...
    conv1.weight = nn.Parameter(
        torch.from_numpy(sobel1).cuda().float().unsqueeze(0).unsqueeze(0),
        requires_grad=True,
    )
...
def sobel_make_transforms(
    ...
    if config.crop_orig:
        tf1_list += [
            torchvision.transforms.RandomCrop(
                tuple(np.array([config.rand_crop_sz, config.rand_crop_sz]))
            ),
            torchvision.transforms.Resize(
                tuple(np.array([config.input_sz, config.input_sz]))
            ),
        ]
        tf3_list += [
    ...
    if cutout or config.cutout:
        ...
        tf2_list.append(
            torchvision.transforms.RandomApply(
                [
                    custom_cutout(
                        min_box=int(config.rand_crop_sz * 0.2),
    ...
    tf2_list += [
        ...
        torchvision.transforms.ColorJitter(
            brightness=0.4, contrast=0.4, saturation=0.4, hue=0.125
        ),
    ]
    ...
    if config.demean:
        ...
        tf1_list.append(
            torchvision.transforms.Normalize(mean=config.data_mean, std=config.data_std)
        )
        tf2_list.append(
            torchvision.transforms.Normalize(mean=config.data_mean, std=config.data_std)
        )
