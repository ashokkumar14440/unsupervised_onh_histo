...
def transfer_images(loader_tuple, config):
    ...
    for index in range(config.dataset.num_dataloaders):
        ...
        images[IMAGES][actual_batch_start:actual_batch_end, :, :, :] = img
        images[TRANSF_IMAGES][actual_batch_start:actual_batch_end, :, :, :] = t_img
        images[INV_TRANSFORM][
            actual_batch_start:actual_batch_end, :, :
        ] = inverse_transform
        images[LABELS][actual_batch_start:actual_batch_end, :, :] = lbl
        ...
        images[LABELS] = images[LABELS][:total_size, :, :]
    return images
...
def _create_image_list(config, channel_count):
    return [
        _create_empty(config, channel_count),
        _create_empty(config, channel_count),
        _create_empty_affine(config),
        _create_empty_mask(config),
    ]
...
def get_dataloader_batch_size(loader_tuple):
    size = loader_tuple[0][0].shape[0]
    for i in range(1, len(loader_tuple)):
        assert size == loader_tuple[0][i].shape[0]
    return size


def get_channel_count(loader_tuple):
    count = loader_tuple[0][0].shape[1]
    assert count == loader_tuple[0][1].shape[1]
    return count
